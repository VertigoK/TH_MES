...
def simulateData(prdInfo, prdTime, plan_qty, specs, tols, k, specs_measured, tols_measured):
    ...
    df.to_csv('.\Test\productionData.csv', index=False)
    ...
    df.to_csv('.\Test\qualityData.csv', index=False)


    # 3. 생산정보 데이터를 DB의 production 테이블에 업로드
    
    input_file = '.\Test\productionData.csv'
    ...
    input_file = '.\Test\qualityData.csv'
    file_reader = csv.reader(open(input_file, 'r'), delimiter=',')
    header = next(file_reader) # 첫 번째 줄 읽기
