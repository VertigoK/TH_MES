...
CREATE TABLE `CUST_ORDER` (
	...
	`DELAYED_DAYS`  INT     NOT NULL DEFAULT 0, -- 납기지연일
	`WO_STATUS`		BOOLEAN NOT NULL DEFAULT FALSE, -- (10/05/2021 추가)
	`OURORDER_STATUS` BOOLEAN NOT NULL DEFAULT 0
);
...
CREATE TABLE `reserved_item` (
  ...
  `plant_cd` int(11) NOT NULL,
  `item_cd` int(11) NOT NULL,
  `plan_qty` int(11) NOT NULL DEFAULT 0,
  `item4_qty` int(11) NOT NULL DEFAULT 0,
  `item5_qty` int(11) NOT NULL DEFAULT 0,
  `item6_qty` int(11) NOT NULL DEFAULT 0,
  `used_yn` tinyint(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`order_no`),
  KEY `FK_PLANT_TO_RESERVED_ITEM` (`plant_cd`),
  KEY `FK_ITEM_TO_RESERVED_ITEM` (`item_cd`),
  CONSTRAINT `FK_CUST_ORDER_TO_RESERVED_ITEM` FOREIGN KEY (`order_no`) REFERENCES `cust_order` (`ORDER_NO`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_ITEM_TO_RESERVED_ITEM` FOREIGN KEY (`item_cd`) REFERENCES `item` (`ITEM_CD`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_PLANT_TO_RESERVED_ITEM` FOREIGN KEY (`plant_cd`) REFERENCES `plant` (`PLANT_CD`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;